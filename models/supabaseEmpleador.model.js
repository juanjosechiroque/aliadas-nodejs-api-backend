/**
 * CMS empleador: public.cms_empleador (PostgREST).
 */

const TABLE = 'cms_empleador';
const PAIR_COUNT = 10;
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_empleador.'
    );
  }
  return k;
}
...
async function fetchEmpleadorById(id) {
...
async function patchEmpleadorFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
