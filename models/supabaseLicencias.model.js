/**
 * CMS licencias: public.cms_licencias (PostgREST).
 */

const TABLE = 'cms_licencias';
const PAIR_COUNT = 21;
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_licencias.'
    );
  }
  return k;
}
...
async function fetchLicenciasById(id) {
...
async function patchLicenciasFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
