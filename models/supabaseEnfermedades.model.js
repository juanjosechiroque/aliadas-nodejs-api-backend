/**
 * CMS enfermedades: public.cms_enfermedades (PostgREST).
 */
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_enfermedades.'
    );
  }
  return k;
}
...
async function fetchEnfermedadesById(id) {
...
async function patchEnfermedadesFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
