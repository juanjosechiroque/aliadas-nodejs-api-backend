/**
 * CMS jornada: public.cms_jornada (PostgREST).
 */

const TABLE = 'cms_jornada';
const PAIR_COUNT = 10;
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_jornada.'
    );
  }
  return k;
}
...
async function fetchJornadaById(id) {
...
async function patchJornadaFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
