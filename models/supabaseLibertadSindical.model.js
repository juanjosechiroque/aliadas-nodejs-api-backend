/**
 * CMS Libertad sindical: public.cms_libertad_sindical (PostgREST).
 * La API expone las mismas claves que la tabla (description, title_n, text_n).
 */
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_libertad_sindical.'
    );
  }
  return k;
}
...
async function fetchLibertadById(id) {
...
async function patchLibertadFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
