/**
 * CMS Beneficios laborales: public.cms_beneficios (PostgREST).
 */

const TABLE = 'cms_beneficios';
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_beneficios.'
    );
  }
  return k;
}
...
async function fetchBeneficiosById(id) {
...
async function patchBeneficiosFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
