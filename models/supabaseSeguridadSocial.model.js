/**
 * CMS seguridad social: public.cms_seguridad_social (PostgREST).
 */

const TABLE = 'cms_seguridad_social';
const PAIR_COUNT = 17;
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_seguridad_social.'
    );
  }
  return k;
}
...
async function fetchSeguridadSocialById(id) {
...
async function patchSeguridadSocialFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
