/**
 * CMS salario: public.cms_salario (PostgREST).
 */

const TABLE = 'cms_salario';
const PAIR_COUNT = 10;
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_salario.'
    );
  }
  return k;
}
...
async function fetchSalarioById(id) {
...
async function patchSalarioFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
