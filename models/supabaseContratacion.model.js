/**
 * CMS Contratación: public.cms_contratacion (PostgREST).
 */

const TABLE = 'cms_contratacion';
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_contratacion.'
    );
  }
  return k;
}
...
async function fetchContratacionById(id) {
...
async function patchContratacionFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
