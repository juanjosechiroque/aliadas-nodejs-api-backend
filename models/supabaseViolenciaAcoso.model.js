/**
 * CMS Violencia y acoso: public.cms_violencia_acoso (PostgREST).
 */

const TABLE = 'cms_violencia_acoso';
...
function writeKey() {
  ...
  if (!k) {
    throw new Error(
      'Falta SUPABASE_SERVICE_ROLE_KEY para actualizar cms_violencia_acoso.'
    );
  }
  return k;
}
...
async function fetchViolenciaById(id) {
...
async function patchViolenciaFromBody(id, body) {
  const url = supabaseUrl();
  const key = writeKey();
  const enc = encodeURIComponent(id);
